...
class MyMinLengthError(MyExceptions):
    ...
    def __str__(self):
        ...
        if self.param < self.value:
            logging.error(f"Сторона треугольника отрицательная: {self.param}")
...
class MyTriangleError(MyExceptions):
    ...
    def __str__(self):
        logging.error(f"Tреугольника со сторонами {self.a}, {self.b}, {self.c} не существует!")
...
class CheckBagSize(MyExceptions):
    ...
    def __str__(self):
        logging.error(f"Предмет размером {self.item} не влезет в сумку размером {self.bag}!")